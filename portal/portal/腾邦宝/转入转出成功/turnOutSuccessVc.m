...
@interface turnOutSuccessVc ()

@end

@implementation turnOutSuccessVc

- (void)viewDidLoad {
    [super viewDidLoad];
    [self setselfArry_data];
    self.title = NSLocalizedString(@"转出详情", @"");
    [self.foot.btn setTitle:@"完成" forState:UIControlStateNormal];
    // Do any additional setup after loading the view.
}
- (void)setheadForTableView{
    OutcashwithdrawalSuccessHead *head = [[OutcashwithdrawalSuccessHead alloc]initWithFrame:CGRectMake(0, 0, 0, 189)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([data.title isEqualToString:NSLocalizedString(@"转出方式", @"")]) {
...
- (void)configureScanpaymentsuccessCell:(ScanpaymentsuccessCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    setUpData *data = self.Arry_data[indexPath.row];
    cell.dataScan = data;
    if ([data.title isEqualToString:NSLocalizedString(@"转出金额", @"")]) {
...
- (void)setselfArry_data{
    ...
    tmp11.title = NSLocalizedString(@"转出金额", @"");
    [self.Arry_data addObject:tmp11];
    
    setUpData *tmp13 = [setUpData new];
    tmp13.title = NSLocalizedString(@"转出方式", @"");
