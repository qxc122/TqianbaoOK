...
#define kDotSize CGSizeMake (12, 12) //密码点的大小
...
- (void)initPwdTextField
{
    ...
    UIColor *color = ColorWithHex(0xCDCCCC, 1.0);
    //生成分割线
    ...
    for (int i = 0; i < kDotCount; i++) {
        ...
        dotView.backgroundColor = ColorWithHex(0x33383F, 1.0);
...
- (void)textFieldDidChange:(UITextField *)textField
{
    ...
    if (textField.text.length == kDotCount && self.Fill_in_the_text) {
        NSLog(@"输入完毕");
        self.Fill_in_the_text(textField.text);
    }
    if (self.Fill_In_input) {
        self.Fill_In_input(textField.text);
    }
}
...
{
    if (!_textField) {
        _textField = [[UITextFieldAdd alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, K_Field_Height)];
        ...
        _textField.layer.borderColor = [ColorWithHex(0xCDCCCC, 1.0) CGColor];
